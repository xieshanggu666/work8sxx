  ...
  <div class="coupon-view">
    ...
    <div v-else>
      <div class="card scan-card">
        <div class="card-title">
          📷 扫码 / 输码核销
          <span class="title-sub">用户在「卡券核销」出示券码，运营输入后校验状态与有效期，一键核销</span>
        </div>
        <div class="scan-row">
          <input
            v-model="codeInput"
...
const allCoupons = computed(() => [...store.coupons].sort((a, b) => b.ts - a.ts))
...
// —— 台账（运营看全量，用户只看本人券与本人预占记录） ——
const visibleLogs = computed(() => {
  if (store.isOperator) return store.couponLogs.slice(0, 60)
  const myCodes = new Set(store.myCoupons.map((c) => c.code))
  const myRecordIds = new Set(
    store.records.filter((r) => (r.userId || store.user.id) === store.user.id).map((r) => r.id)
  )
  return store.couponLogs
